import { MessageCircle, Phone, Mail, MapPin, Facebook, Instagram, Twitter, Linkedin, Youtube } from "lucide-react";
import { useAppContext } from "../context/AppContext";

export default function Footer() {
  const { lang, socialLinks } = useAppContext();
  ...
  return (
    <footer style={{ background: "#060f1e", borderTop: "1px solid rgba(201,168,76,.08)", padding: "3.25rem 5% 1.6rem" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: "2.25rem", marginBottom: "2.25rem" }}>
          <div>
          ...
          <div style={{ display: "flex", gap: ".65rem" }}>
            {socialIcons.map(({ Icon, url, label }, i) => (
              <a
                ...
                aria-label={label}
                style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(201,168,76,.08)", border: "1px solid rgba(201,168,76,.2)", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--gold)", cursor: "pointer", transition: "all .3s", textDecoration: "none" }}
                onMouseEnter={e => { e.currentTarget.style.background = "var(--gold)"; e.currentTarget.style.color = "var(--navy)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "rgba(201,168,76,.08)"; e.currentTarget.style.color = "var(--gold)"; }}
              >
                <Icon size={12} />
              </a>
            ))}
